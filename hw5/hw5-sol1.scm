...
(define match-junk
	(lambda (k frag)
		(or (call/cc 
				(lambda (backtracker) (cons (lambda () (backtracker #f)) frag)))
		(and (< 0 k)
			(pair? frag)
...
(define match-*
	(lambda (matcher frag)
		(or (call/cc (lambda (backtracker) 
				(cons (lambda () (backtracker #f)) frag))) 
			(let ((head (matcher frag)))
				(if head
					(and (not (eq? (cdr head) frag))
		...
		(cond
			((symbol? pat)
				(lambda (frag)
					(and (pair? frag)
						 (eq? pat (car frag))
						 (call/cc (lambda (backtracker) (cons (lambda () (backtracker #f)) (cdr frag)))))))

			((eq? 'or (car pat))
				(let make-or-matcher ((pats (cdr pat)))
					(if (null? pats)
						(lambda (frag) (call/cc (lambda (backtracker) (cons (backtracker #f) frag)))) 			
						(let ((head-matcher (make-matcher (car pats)))
							...
							(lambda (frag) 
								(let ((head (head-matcher frag)))
									(if head
										(or (call/cc (lambda (backtracker)
												(cons (lambda () (backtracker #f)) (cdr head))))
											(tail-matcher frag))
										(tail-matcher frag))))))))
				...
				(let make-list-matcher ((pats (cdr pat)))
					(if (null? pats)
						...
						(lambda (frag)
								(let ((head (head-matcher frag)))
									(if head
										(let ((tail (tail-matcher (cdr head))))
											(if tail
												tail 
												((car head))))
										#f )))))))
